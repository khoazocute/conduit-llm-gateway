...
- **Lịch dùng key thật theo phase** (`docs/roadmap.md`): B smoke test 1 lượt ngắn/model; C–D chạy thử nhỏ (≈27 lượt); E chạy chính thức 270 lượt (đánh giá proxy, **không có RAG** — 30 prompt là câu hỏi tổng quát, không có bước retrieval).
...
- **D3 (đã chốt 2026-09-23):** đường ghi log thí nghiệm đi qua app Conduit thật (`POST /agents/{id}/chat`), không gọi thẳng 3 proxy — khớp lời dặn của thầy Bình ("áp dụng proxy đó cho dự án của em").
- **D4 (đã chốt 2026-09-23):** 6 prompt code chấm rubric 3 tiêu chí như prompt mở (không chấm bằng test tự động).
- **D5 (tạm chốt 2026-09-23):** giá Gemini dùng giá niêm yết; Khoa sẽ chốt lại toàn bộ `model_pricing` sau.
- **D6 (đã chốt 2026-09-23):** 30% mẫu chấm kép tính theo lượt output (~49/162), không theo prompt.
- **D2, D7, D8:** còn trống, xem `docs/roadmap.md` mục 4 và 6 (câu hỏi cho GVHD).

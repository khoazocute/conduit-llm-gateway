...
public class PurchaseController {
    ...
    /** The caller's library: agents they have paid for. */
    @GetMapping("/purchases")
    public PurchasedAgentListResponse listMyPurchases(
            @AuthenticationPrincipal UserPrincipal principal,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "20") int size) {
        return purchaseService.listPurchasedAgents(principal.getId(), PageRequest.of(page, size));
    }
